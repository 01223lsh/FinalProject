...
function validate() {
	...
	var objPw2 = document.getElementById("pw2"); //비밀번호확인
  	...
  	var objEmail = document.getElementById("sm_email"); //이메일
  	var objPhone = document.getElementById("phone"); //핸드폰번호
  	var objBankName = document.getElementById("bankName"); //은행명
  	var objACN = document.getElementById("accountNumber"); //계좌번호
  	var objAN = document.getElementById("accountName"); //예금주
	...
	//은행명 유효성검사(4~5자 한글만 가능)
	var bn_RegExp = /^[가-힣]{4,5}$/;
	//계좌번호 유효성검사(10~30자 숫자만 가능)
	var acn_RegExp = /^[0-9]{10,30}$/;
	//예금주 유효성검사(2~10자 한글,숫자,영어만 가능)
	var an_RegExp = /^[0-9가-힣a-zA-Z]{2,10}$/;
	...
	//패스워드와 ID가 동일한지 검사
	 if((objPw.value) == (objId.value)){ 
	  ...
	  // ================ bankName 유효성검사 ================ //
	  if((bn_RegExp.value) == ''){ // 은행명 입력여부 검사
	      alert("은행명을 입력해주세요.");
	      objBankName.focus();
	      return false;
	  }
	  
	  if(!check(bn_RegExp,objBankName,"은행명을 잘못 입력 했습니다.")) {
	      return false;
	  }
	  
	  // ================ accountNumber 유효성검사 ================ //
	  if((acn_RegExp.value) == ''){ // 계좌번호 입력여부 검사
	      alert("계좌번호 입력해주세요.");
	      objACN.focus();
	      return false;
	  }
	  
	  if(!check(acn_RegExp,objACN,"계좌번호를 잘못 입력했습니다. 형식을 확인해주세요.(숫자만 입력 가능)")) {
	      return false;
	  }
	  
	  // ================ accountName 유효성검사 ================ //
	  if((an_RegExp.value) == ''){ // 예금주 입력여부 검사
	      alert("예금주 입력해주세요.");
	      objAN.focus();
	      return false;
	  }
	  
	  if(!check(an_RegExp,objAN,"예금주는 2~10자의 한글, 영어, 숫자로만 입력 가능합니다.")) {
	      return false;
	  }
...
<div class="form-group">
	...
	<div class="col-sm-5">
		<div id="emailform">
			<input type="text" class="form-control" id="sm_email" name="sm_email" placeholder="이메일을 입력해주세요(ex. funding@naver.com)" maxlength="20" >
...
<div class="form-group">
	...
	<div class="col-sm-5">
		<input type="text" id="sm_email2" name="sm_email2" title="인증번호 입력" disabled /> 
...
<div class="form-group">
	<div>
		...
		<input type="reset" id="cancel" class="btn btn-danger" value="약관동의'로 이동" />
	</div>
</div>
