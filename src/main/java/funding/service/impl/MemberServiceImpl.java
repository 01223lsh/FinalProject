...
//	public ArrayList<String> findId(Member member) {
	public String findId(Member member) {
		...
		/*
		 * 배열로 값을 받아올 경우 List<Member> list = memberDao.findId(member); ArrayList<String>
		 * findId = new ArrayList<String>(); for(int i =0; i <list.size(); i ++) {
		 * String id = list.get(i).getId(); findId.add(id);
		 */
